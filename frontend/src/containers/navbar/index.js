...
const NavBar = () => {
  ...
  const[logged,seLogged]=useState(true)// Pour changer l'apparence du navBar lorsqu'un utilisateur se connecte à son compte
...
{  Active  ? 
        <> {!signIn ? <>
						...
						<div className='w-full h-full flex justify-center items-center fixed z-20'>
							<Login handle={handleClick} handleCon={handleSeConnecter}>
							</Login>
						</div>
				    </> 
				:   <>
						...
						<div className='w-full h-full flex justify-center items-center fixed z-20'>
							<Register handle={handleClick} handleSinscr={handleSInscrire}>
							</Register>
						</div>
				    </>}
         </>: <></> }
